document.addEventListener(`DOMContentLoaded`, function(){
    ...
    document.addEventListener(MAIN_MENU, function(){
        ...
        gameTitle.innerText = `Confirm You are Human .`;
        
        const playBtn = document.createElement(`button`);
        playBtn.innerText = `Proceed`;
        playBtn.addEventListener(`click`, function(){
            loadingBeforePlay(menuBox);
        });

        const bestTime = document.createElement(`p`);
        // if(60-localStorage.getItem(STORAGE_KEY)===60){
    ...
    const showGameResult = function(param){
        ...
        if(param === true){
            menuBoxText.innerText = `You Did It !!`;
        ...
        confirmBtn.innerText = `Hive Tower >>`;

        const btnWrapper = document.createElement(`a`);
        btnWrapper.setAttribute(`href`, `../Chapter 5/chapter5.html`);
        btnWrapper.classList.add(`btnWrapper`);
        btnWrapper.append(confirmBtn);

        const bestTime = document.createElement(`p`);
        bestTime.innerText = `Best Time: ${60-localStorage.getItem(STORAGE_KEY)}s`;
